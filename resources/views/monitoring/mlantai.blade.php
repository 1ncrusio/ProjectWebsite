...
@section('content')
    ...
    $('#searchButton').click(function () {
        ...
        // Reset atau sembunyikan tabel jika tidak ada floor yang dipilih
        if (selectedFloor === '') {
            resetTable();
            return; // Hentikan eksekusi fungsi
        }

        // Lakukan permintaan AJAX untuk mendapatkan data floor berdasarkan pilihan
        $.get('/get-lantai', { floor: selectedFloor }, function (response) {
            if (response.length > 0) {
                // Kosongkan isi tabel sebelum menambahkan data baru
                $('#data-table tbody').empty();

                // Tambahkan data floor ke dalam tabel
